...
Escopo:
- Seu único papel é entender a necessidade de seguros ou benefícios do visitante, coletar os campos necessários para a qualificação comercial e preparar o encaminhamento a um especialista da Luminar.
- Você pode fazer perguntas de qualificação e explicar, em termos gerais, que preços, coberturas, aceitação e condições dependem da análise de um corretor.
- Não responda sobre assuntos fora de seguros, benefícios e qualificação comercial. Não forneça cotações, preços, condições, coberturas específicas, seguradoras, aprovação, aconselhamento jurídico, médico ou financeiro, nem informações internas da Luminar que não estejam neste prompt.

Honestidade e fontes:
- Use como fonte de verdade somente as mensagens da conversa e o estado estruturado validado fornecido pelo servidor.
- Nunca invente, complete por plausibilidade, presuma ou transforme ausência de informação em uma resposta negativa.
- Se você não sabe a resposta ou a informação não está no contexto fornecido, diga exatamente "Não sei com base no contexto disponível." em vez de inventar e solicite o encaminhamento para um especialista. Não continue a qualificação nem faça outra pergunta.
- Antes de produzir reply, collected ou qualification.specialistSummary, verifique se cada afirmação factual foi informada explicitamente pelo visitante. Se não puder vinculá-la a uma mensagem, remova-a ou use null.
...
- A pergunta de reply deve tratar exclusivamente de nextField. Nunca pergunte observações adicionais, detalhes opcionais ou se o visitante quer acrescentar algo.
...
- Respostas curtas como "não", "sim" ou "nenhum" respondem somente à pergunta imediatamente anterior. Não deduza outros fatos nem associe a resposta a tópicos que não foram perguntados.
- Quando não houver campos obrigatórios ausentes, encerre imediatamente com mode qualified. Não peça confirmação, permissão para prosseguir, mais detalhes ou novas observações.
- Se o visitante pedir "análise", "finalizar", "concluir" ou equivalente, não inicie uma nova etapa: preencha apenas dados explicitamente informados e encaminhe para qualificação assim que os campos obrigatórios estiverem completos.
  ...
  properties: {
    ...
    collected: {
      ...
      properties: Object.fromEntries(
        FIELD_NAMES.map((field) => [field, nullableString]),
      ),
    },
    missingFields: {
      type: 'array',
...
const QUESTIONS: Record<FieldName, string> = {
  profile:
    'Para direcionar melhor, você busca proteção como pessoa física ou para uma empresa?',
  interest: 'Qual tipo de seguro ou benefício você procura neste momento?',
  age: 'Qual é a sua idade?',
  peopleToInclude:
    'Quem você deseja incluir nessa proteção e quais são as idades aproximadas?',
  collaborators: 'Quantos colaboradores a empresa possui aproximadamente?',
  currentCoverage:
    'Você já possui atualmente algum plano ou seguro relacionado a essa necessidade?',
  ...
  vehicleDetails:
    'O veículo é novo ou usado e será utilizado para fins pessoais ou profissionais?',
  propertyDetails:
    'O imóvel é próprio ou alugado e possui alguma proteção atualmente?',
};
...
function isAgentState(value: unknown): value is AgentState {
  ...
  if (
    (state.mode !== 'collecting' && state.mode !== 'qualified') ||
    typeof state.reply !== 'string'
  )
    return false;
  if (
    !state.collected ||
    ...
    !state.qualification ||
    typeof state.qualification !== 'object'
  )
    return false;
  if (
    !FIELD_NAMES.every(
      (field) =>
        ...
        typeof state.collected?.[field] === 'string',
    )
  )
    return false;
  const validPriority =
    state.qualification.priority === null ||
    ['Baixa', 'Média', 'Alta'].includes(String(state.qualification.priority));
  const validSummary =
    state.qualification.specialistSummary === null ||
    typeof state.qualification.specialistSummary === 'string';
  return (
    ...
    (state.nextField === null ||
      (state.nextField !== undefined &&
        FIELD_NAMES.includes(state.nextField))) &&
    validPriority &&
    validSummary
  );
}
...
function hasValue(value: string | null): boolean {
  return (
    typeof value === 'string' &&
    value.trim().length > 0 &&
    !/^(a confirmar|não informado)$/i.test(value.trim())
  );
}

function includesAny(value: string | null, patterns: RegExp[]): boolean {
  return (
    hasValue(value) && patterns.some((pattern) => pattern.test(value as string))
  );
}
...
function requiredFields(data: CollectedData): FieldName[] {
  ...
  const company = includesAny(data.profile, [
    /empresa/i,
    /jurídica/i,
    /empresarial/i,
  ]);
  ...
  const residential = includesAny(data.interest, [
    /resid/i,
    /imóvel/i,
    /casa/i,
    /apartamento/i,
  ...
  if (person && (health || life))
    required.push('peopleToInclude', 'currentCoverage');
  if (person && auto) required.push('vehicleDetails', 'currentCoverage');
  if (person && residential)
    required.push('propertyDetails', 'currentCoverage');
...
function label(field: FieldName): string {
  const labels: Record<FieldName, string> = {
    ...
    interest: 'Interesse',
    age: 'Idade',
    peopleToInclude: 'Pessoas a incluir',
    collaborators: 'Colaboradores',
    currentCoverage: 'Plano ou seguro atual',
...
function formatQualification(state: AgentState): string {
  ...
  const summary =
    state.qualification.specialistSummary ??
  ...
  return [
    ...
    `Resumo para o especialista: ${summary}`,
  ].join('\n');
}

function questionMatchesField(reply: string, field: FieldName): boolean {
  const patterns: Record<FieldName, RegExp> = {
    profile: /pessoa\s+f[ií]sica|empresa|empresarial|jur[ií]dica/i,
    interest:
      /tipo de seguro|benef[ií]cio|prote[cç][aã]o|o que (?:voc[eê] )?(?:procura|busca)/i,
    age: /idade|quantos anos/i,
    peopleToInclude:
      /quem.*incluir|dependente|benefici[aá]ri|pessoas?.*prote[cç][aã]o/i,
    collaborators:
      /quantos?.*(?:colaboradores|funcion[aá]rios)|n[uú]mero de colaboradores/i,
    currentCoverage:
      /j[aá].*(?:plano|seguro|prote[cç][aã]o)|possui.*(?:plano|seguro|prote[cç][aã]o)|cobertura atual/i,
    location: /cidade|estado|localiza[cç][aã]o|\bUF\b/i,
    situation:
      /situa[cç][aã]o atual|como est[aá].*(?:hoje|atualmente)|cen[aá]rio atual/i,
    need: /principal necessidade|dificuldade|problema.*resolver|o que precisa resolver/i,
    objective:
      /objetivo|resultado.*(?:espera|busca|pretende)|o que espera alcan[cç]ar/i,
    urgency: /quando.*(?:resolver|precisa|pretende)|prazo|urg[eê]ncia/i,
    vehicleDetails:
      /ve[ií]culo.*(?:novo|usado)|novo ou usado|uso.*(?:pessoal|profissional)|finalidade de uso/i,
    propertyDetails:
      /im[oó]vel.*(?:pr[oó]prio|alugado)|pr[oó]prio ou alugado|prote[cç][aã]o.*im[oó]vel/i,
  };
  return patterns[field].test(reply);
}

function indicatesUnknown(reply: string): boolean {
  return /não sei(?: responder)?(?: isso)?(?: com segurança)?(?: com base no contexto disponível)?/i.test(
    reply,
  );
}
...
function safeCollectingReply(reply: string, nextField: FieldName): string {
  ...
  const containsInternalLanguage =
    /(?:missingFields|nextField|peopleToInclude|currentCoverage|vehicleDetails|propertyDetails|JSON|campo obrigatório)/i.test(
      normalized,
    );
  const containsPrematureAnalysis =
    /ANÁLISE LUMINAR SMART|Prioridade comercial:|Resumo para o especialista:/i.test(
      normalized,
    );
  ...
  if (
    ...
    !questionMatchesField(normalized, nextField)
  ) {
...
function shouldRetry(status: number, code: string | null): boolean {
  if (code === 'insufficient_quota' || status === 401 || status === 403)
    return false;
...
function isShortAnswer(content: string): boolean {
  return /^(?:n[aã]o|sim|nenhum(?:a)?|nada|n[aã]o tenho|n[aã]o possui|correto|isso)$/i.test(
    content.trim(),
  );
}
...
function mergeCollected(
  ...
  const shortAnswer =
    previous?.mode === 'collecting' && isShortAnswer(latestUserMessage);
  const answeredField =
    shortAnswer &&
    previous.nextField &&
    questionMatchesField(previousAssistantMessage, previous.nextField)
      ? previous.nextField
      : null;

  return Object.fromEntries(
    FIELD_NAMES.map((field) => {
      if (shortAnswer && field !== answeredField) {
        return [field, previous?.collected[field] ?? null];
      }
      return [
        field,
        hasValue(current[field])
          ? current[field]
          : (previous?.collected[field] ?? null),
      ];
    }),
  ) as CollectedData;
}
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    return json({ error: 'A mensagem enviada não é válida.' }, 400);
  }
  if (
    !body ||
    typeof body !== 'object' ||
  ...
  if (
    ...
    return json(
      { error: 'A conversa excede os limites desta demonstração.' },
      400,
    );
  }
  ...
  try {
    ...
    if (!upstream.ok) {
      ...
      return json(
        {
          error: 'O atendimento inteligente está temporariamente indisponível.',
        },
    ...
    if (!isAgentState(parsed)) {
      ...
      return json(
        { error: 'O assistente não retornou um estado válido.' },
        502,
      );
    }
    ...
    parsed.collected = mergeCollected(
      parsed.collected,
      previousState,
      latestUserMessage,
    ...
    if (indicatesUnknown(parsed.reply)) {
      const state = { ...parsed, mode: 'collecting' as const };
      return json({ message: UNKNOWN_HANDOFF_MESSAGE, state }, 200);
    }

    const required = requiredFields(parsed.collected);
    const missingFields = required.filter(
      (field) => !hasValue(parsed.collected[field]),
    );
    if (missingFields.length > 0) {
      const nextField =
        parsed.nextField && missingFields.includes(parsed.nextField)
          ? parsed.nextField
          : missingFields[0];
      const state = {
        ...parsed,
      ...
      return json(
        { message: safeCollectingReply(parsed.reply, nextField), state },
        200,
      );
    }
